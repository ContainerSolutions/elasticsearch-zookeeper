...
public class ZooKeeperDiscovery extends AbstractLifecycleComponent<Discovery> implements Discovery, DiscoveryNodesProvider {
    ...
    @Override public void publish(ClusterChangedEvent clusterChangedEvent, AckListener ackListener) {
        ...
        try {
            ...
            latestDiscoNodes = newClusterState.nodes();
            statePublisher.publish(clusterChangedEvent, ackListener);
    ...
    private void updateNodeList(final Set<String> nodes) {
        clusterService.submitStateUpdateTask("zoo-keeper-disco-update-node-list", Priority.IMMEDIATE, new ClusterStateNonMasterUpdateTask() {
            ...
            public ClusterState execute(ClusterState currentState) {
                try {
                    final Set<String> currentNodes = new HashSet<>();
                    UnmodifiableIterator<String> iterator = latestDiscoNodes.nodes().keysIt();
                    while (iterator.hasNext()) {
                        currentNodes.add(iterator.next());
                    }
        ...
        if (!master) {
            ...
            if (clusterState.nodes().localNode() != null) {
                ...
                if (logger.isTraceEnabled()) {
                    ...
                    sb.append(clusterState.getRoutingNodes().prettyPrint());
    ...
    public DiscoveryNode nodeInfo(final String id) throws ElasticsearchException, InterruptedException {
        try {
            ...
            if (buf != null) {
                ...
                return DiscoveryNode.readNode(new ByteBufferStreamInput(bb));
            } else {
                return null;
            }
        } catch (IOException e) {
    ...
    private class ZooKeeperStatePublisher implements StatePublisher {
        ...
        @Override public void publish(ClusterChangedEvent clusterChangedEvent, AckListener ackListener) {
            try {
                ...
                zooKeeperClusterState.publish(clusterChangedEvent, ackListener);
            } catch (InterruptedException ex) {
                Thread.currentThread().interrupt();
            }
